...
function NeptuneOrbit() {
  ...
  const points = useMemo(() => {
    ...
    const neptuneOrbitalPeriod = 60190; // Neptune takes about 60,190 Earth days to orbit the Sun

    for (let i = 0; i < neptuneOrbitalPeriod * 2; i++) { // Sample every 12 hours
      const currentDate = new Date(startDate.getTime() + i * halfDayInMs);
      const neptunePosition = HelioVector(Body.Neptune, currentDate);
